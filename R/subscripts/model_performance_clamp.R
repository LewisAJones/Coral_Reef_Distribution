...
files <- list.files("./results/Predictions/Clamping/Clamped/LPT/", pattern = ".asc")
files <- files[!files == "Modern.asc"]
master <- data.frame()

for(i in files){
  #load rasters
  r <- raster(paste("./results/Predictions/Clamping/Clamped/LPT/", i, sep =""))
  #extract stage name
...
write.csv(master, "./results/Performance/LPT_clamped_results.csv", row.names = FALSE)

#maxSSS

files <- list.files("./results/Predictions/Clamping/Clamped/MaxSSS/", pattern = ".asc")
files <- files[!files == "Modern.asc"]
master<- data.frame()

for(i in files){
  #load rasters
  r <- raster(paste("./results/Predictions/Clamping/Clamped/MaxSSS/", i, sep =""))
  #extract stage name
  name <- tools::file_path_sans_ext(i)
  #subset collection data
  xy <- subset(collections, stage == name)[,c("x", "y")]
...
write.csv(master, "./results/Performance/MaxSSS_clamped_results.csv", row.names = FALSE)
