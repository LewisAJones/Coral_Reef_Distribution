...
p1 <- ggplot(data = data) +
  geom_rect(data = periods, mapping=aes(xmin=min(data$min_ma), xmax=0, ymin= -35, ymax= -30), linetype = 1, colour = "black", fill="black", alpha=1)  +
  geom_rect(data = throwing_shade, aes(xmin=min_ma, xmax=max_ma, ymin=-35, ymax=35), linetype = 0, color= "grey95", alpha=0.2)  +
  geom_rect(data = periods, mapping=aes(xmin=min_ma, xmax=max_ma, ymin= -35, ymax= -30), linetype = 1, colour = "black", fill=periods$color, alpha=1)  +
  geom_text(data = periods, mapping=aes(x=(min_ma+max_ma)/2, y= -32.5, label = abbrev), colour = "black", alpha=1)  +
  ...
  scale_y_continuous(expand = c(0,0), limits = c(-35, 35), breaks = seq(-30, 30, 15), labels = seq(-30, 30, 15)) +
...
p2 <- ggplot(data = data) +
  ...
  theme(panel.background = element_blank(),
        ...
        aspect.ratio = 0.5)
p2
ggsave("./figures/centroid_plot_MaxSSS.png", plot = p2, width = 200, height = 110, units = "mm", dpi = 600)

p2 <- p2 + labs(subtitle = "MaxSSS")

p <- ggarrange(p1, p2, ncol=1, nrow=2, labels = "AUTO", align = "v", font.label = list(size = 18))
p
ggsave("./figures/centroid_plot.png", plot = p, width = 65, height = 75, units = "mm", dpi = 600, scale = 3)
