...
occ <- read.csv("./data/occurrences/subsampled_PBDB_collections_extract.csv")
occ <- left_join(x = occ, y = stages, by = c("stage" = "interval_name"))
#---------------------------------
...
p1 <- ggplot(data = data) +
  geom_rect(data = periods, mapping=aes(xmin=min(data$min_ma), xmax=0, ymin= -70, ymax= -60), linetype = 1, colour = "black", fill="black", alpha=1)  +
  ...
  geom_point(data = occ, aes(x = mid_ma, y = y, shape = "Reef localities"), colour = "#0c2c84", size = 1.5, alpha = 1) +
  #geom_ribbon(aes(x = mid_ma, ymin = LPT_min, ymax = LPT_max, colour = "Max"), fill = NA, size = 1, alpha = 0.75) +
  #geom_ribbon(aes(x = mid_ma, ymin = LPT_lower, ymax = LPT_upper, colour = "95% quantiles"), fill = NA, linetype = 2, size = 1, alpha = 0.75) +
...
p
ggsave("./figures/reef_zone_plot.png", plot = p, width = 65, height = 75, units = "mm", dpi = 600, scale = 3)
