...
#data <- left_join(x = data, y = stages, by = c("name" = "interval_name"))
#data <- data[order(data$mid_ma),]
#write.csv(data, "./results/Performance/LPT_results.csv", row.names = FALSE)
...
p1 <- ggplot(data = data) +
  geom_rect(data = periods, mapping=aes(xmin=min(data$min_ma), xmax=0, ymin= -8, ymax= 0), linetype = 1, colour = "black", fill="black", alpha=1)  +
  geom_rect(data = throwing_shade, aes(xmin=min_ma, xmax=max_ma, ymin=-8, ymax=115), color= "grey95", alpha=0.2)  +
  geom_rect(data = periods, mapping=aes(xmin=min_ma, xmax=max_ma, ymin= -8, ymax= 0), colour = "black", fill=periods$color, alpha=1)  +
  geom_text(data = periods, mapping=aes(x=(min_ma+max_ma)/2, y= -4, label = abbrev), colour = "black", alpha=1)  +
  ...
  #geom_text_repel(mapping=aes(x=(min_ma+max_ma)/2, y= perf, label = success+failure), fontface = "bold", size = 3, colour = "black", nudge_y = 5, alpha=1)  +
  scale_x_reverse(expand=c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(-8, 115), breaks = seq(0, 100, 25)) +
  scale_colour_manual(values = cols, breaks = labels) +
  labs(x = "", y = "Predictive performance (%)", subtitle = "LTP") +
...
#ggsave("./figures/performance_plot1.png", plot = p1, width = 65, height = 35, units = "mm", dpi = 600, scale = 3)

data <- read.csv("./results/Performance/MaxSSS_results.csv")
#data <- left_join(x = data, y = stages, by = c("name" = "interval_name"))
#data <- data[order(data$mid_ma),]
#write.csv(data, "./results/Performance/MaxSSS_results.csv", row.names = FALSE)
...
p2 <- p2 + labs(subtitle = "MaxSSS")


p <- ggarrange(p1, p2, ncol=1, nrow=2, labels = "AUTO", align = "v", font.label = list(size = 18), common.legend = FALSE)
p
ggsave("./figures/performance_plot.png", plot = p, width = 65, height = 75, units = "mm", dpi = 600, scale = 3)
